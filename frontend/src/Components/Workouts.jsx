...
return ( 
    <div className='workouts'>
        ...
        <br/>
        <div>
        <input className='workouts' type='text' placeholder='Workout Name' id='workoutTitle'></input> {/* use set input as a guid, will need a useState*/}
        </div>
        <br/>
        <br/>
        {exercise.map((ex, idx)=>(
            <Workout getExercises={getExercises} ex={ex} idx={idx} setList={setList}/>
        ...
        <h5>
        Movement: <input type='text' placeholder='Movement Name' id='name' onChange={(event) => setExerciseName(event.target.value)}></input> 
        <MDBBtn onClick={() => setIsClicked(false)} className='me-1' color='danger'>Delete Exercise</MDBBtn>
        <MDBBtn onClick={() => [createExercise(exerciseName), setIsCreated(true), setIsClicked(false)]} className='me-1' style={{ backgroundColor: '#25d366' }}>Create Exercise</MDBBtn>
        </h5>
    ...
    <br/>
    {isCreated ? <MDBBtn onClick={addSet} className='mx-2' color='secondary'>Add Set</MDBBtn> : null} <br/><br/>
        </div>
        :
        null
        }
    ...
    <div className="workouts">
       <Link to='/exercises'><MDBBtn style={{ backgroundColor: '#25d366' }}>Add Exercises</MDBBtn></Link> <br/>
       <MDBBtn onClick={() => setIsClicked(true)} style={{ backgroundColor: '#ff0852' }} >Manually Add Exercises</MDBBtn> <br/>
       <MDBBtn onClick={(event) => saveWorkout(event.target.value)} style={{ backgroundColor: '#0e56f0' }}>Save Workout</MDBBtn>
     </div>
